...
First lets have a look at the extent of the marine park boundary. We can get the extent of an object using `ext()`. We need to convert this into a `SpatVector` object for plotting using `vect()`
...
We crop using the `crop()` function, with raster we want to crop first and the vector we are cropping with second.
