...
ras_near <- project(ras, "ESRI:54009", meth = "near")
pal <- RColorBrewer::brewer.pal(ncell(ras_near), name = "BrBG")

plot(ras_near, type = "classes", col = pal, ext = ext(polys))
plot(polys, "lyr.1", type = "classes")

tmap::tm_shape(ras, bbox = sf::st_bbox(polys)) + tmap::tm_raster(palette = "BrBG", style = "cat") + tmap::tm_shape(sf::st_as_sf(polys)) + tmap::tm_borders()
```
...
```{r}
zones_data_df <- extract(sst_cropped_masked, zones) #the raster we are extracting from first, the polygons we want data for is second
...
Now get the mean temperature in our zones
...
We've got a data frame with each column showing the mean temperature in each zone for one month. However, this format is not great for plotting. 

```{r}
zones_mean_temp_t <- t(zones_mean_temp)

plot_data <- cbind(names = rownames(zones_mean_temp_t), zones_mean_temp_t)
```


We can now plot a time series of temperature in our zones:

```{r}
plot(plot_data$names, plot_data$V1)
```
