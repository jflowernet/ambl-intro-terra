...
Let's pause our raster work for a brief dive into coordinate reference systems.
...
```{r fig.width=8, echo=FALSE, fig.cap="a) Cylindrical; b) conical, and; c) planar projecions"}
...
Coordinate reference systems are a huge topic to cover, so I have only covered some key ideas. The following resources are useful for understanding in more depth:
